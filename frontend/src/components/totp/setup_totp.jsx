...
function SetupTotp() {
  ...
  return (
    <div className="setup_totp flex flex-col items-center justify-center min-h-screen p-4">
      ...
      {totpSecret ? (
        <>
          <p>Scan the QR code with your authenticator authenticator:</p>
          <QRCodeCanvas value={`otpauth://totp/MyApp?secret=${totpSecret}`} size={256} />

          <p>Enter the code generated by your app:</p>
          <input type="text" placeholder="Enter the 6-digit code" className="input-field w-72 p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
          <button onClick={handleSubmit} className="btn mt-4 w-72 bg-blue-500 text-white py-2 px-4 rounded-md hover:bg-blue-600 transition duration-200">
            Verify Code
          </button>
        </>
      ) : (
